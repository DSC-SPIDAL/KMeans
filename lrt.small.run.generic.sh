...
# with IB
mpirun --map-by ppr:$ppn:node:PE=$pe -hostfile $hostfile -np $(($nodes*$ppn)) java $opts -cp $cp org.saliya.ompi.kmeans.ProgramLRT -n $n -d $d -k $k -t $t -c $c -p $p -m $m -b $b -o out.txt -T $T 2>&1 | tee "$pat"_"$n"_"$k"_"$d"_"$m".txt
