...
for cen in ${centers[@]}; do
  ...
  if [ $procbind = "core" ]; then
      ...
      mpirun $btl --report-bindings --map-by ppr:$ppn:node:PE=$pe --bind-to core -hostfile $hostfile -np $(($nodes*$ppn)) java $opts -cp $cp org.saliya.ompi.kmeans.Program -n $n -d $d -k $k -t $t -c $c -p $p -m $m -b $b -o out.txt -T $T -bind $explicitbind 2>&1 | tee fj_"$pat"_"$n"_"$k"_"$d"_"$m".txt
  elif [ $procbind = "socket" ]; then
     # with IB and bound to socket
     mpirun $btl --report-bindings --map-by ppr:$ppn:node --bind-to socket -hostfile $hostfile -np $(($nodes*$ppn)) java $opts -cp $cp org.saliya.ompi.kmeans.Program -n $n -d $d -k $k -t $t -c $c -p $p -m $m -b $b -o out.txt -T $T -bind $explicitbind 2>&1 | tee fj_"$pat"_"$n"_"$k"_"$d"_"$m".txt
  else
      ...
      mpirun $btl --report-bindings --map-by ppr:$ppn:node --bind-to none -hostfile $hostfile -np $(($nodes*$ppn)) java $opts -cp $cp org.saliya.ompi.kmeans.Program -n $n -d $d -k $k -t $t -c $c -p $p -m $m -b $b -o out.txt -T $T -bind $explicitbind 2>&1 | tee fj_"$pat"_"$n"_"$k"_"$d"_"$m".txt
  fi
done
